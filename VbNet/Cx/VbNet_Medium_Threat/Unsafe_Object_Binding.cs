...
if(controllersMethodDecls.Count > 0){	
	...
	CxList dbUpdateMethods = methods.FindByShortNames(new List<string> {"SaveChanges", "SaveChangesAsync"}, false);
	CxList dbUpdateMethodDecl = dbUpdateMethods.GetAncOfType(typeof(MethodDecl)) * controllersMethodDecls;

	//Sanitized by Bind
	CxList bindMethod = All.FindByType(typeof(CustomAttribute)).FindByShortName("Bind", false);
	dbUpdateMethodDecl -= bindMethod.GetAncOfType(typeof(MethodDecl));

	//Sanitized by TryUpdateModel
	CxList tryUpdateMethod = methods.FindByShortName("TryUpdateModel", false);
	...
	CxList updateModelMethod = methods.FindByShortName("UpdateModel", false);
	...
	foreach(CxList updateModel in updateModelMethod){
		...
		foreach(CxList currentParam in allParams){
			CxList unknownRefParamDef = All.FindDefinition(currentParam);
			CxList defType = All.FindByFathers(unknownRefParamDef.GetAncOfType(typeof(VariableDeclStmt))).FindByType("String", false);
			if(defType.Count > 0 && defType.TryGetCSharpGraph<TypeRef>().IsRankSpecifierCollectionExists()){
				arrayParams.Add(currentParam);
			}
		}
