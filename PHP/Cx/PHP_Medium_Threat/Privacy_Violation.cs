...
CxList realLiteral = All.FindByType(typeof(RealLiteral));
...
outputs -= All.GetParameters(Find_Methods().FindByShortName("fwrite"), 0);
...
variableRef -= variableRef.InfluencedByAndNotSanitized(sanitize, Find_DB_Out()).GetStartAndEndNodes(CxList.GetStartEndNodesType.EndNodesOnly);
