...
if (!exit)
{
	...
	if(notInTry.Count > 0)
	{
		...
		if(invokes.Count == 0 || Find_Improper_Exception_Handling(invokes, oTag).Count > 0)
		{
			result.Add(notInTry);
		}
	}
...
// Remove Methods declared with throws
result -= result.GetByAncs(Find_Throws_Methods());