...
foreach (CxList bundleActivator in bundleActivators) {
	...
	if (bundleActivatorStr != null) {
		...
		startStopMethods.Add(activatorClassMethods.FindByShortNames(new List<string> {"start","stop"}));
	}
}
...
allMethods.Add(inputMethods);
allMethods.Add(startStopMethods);

result = All.GetParameters(allMethods);