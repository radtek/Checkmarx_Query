...
// isValid*
CxList isValid = getESAPI.FindByMemberAccess("Validator.isValid*");
isValid.Add(getESAPI.FindByMemberAccess("ValidationRule.isValid*"));

// Find all "isValid* methods in conditions, or conditions influenced by isValid*
CxList conditions = Find_Conditions();
CxList isValidInCondition = isValid.GetByAncs(conditions);
isValidInCondition.Add(conditions.DataInfluencedBy(isValid));
...
result = getESAPI.FindByMemberAccess("Encoder.encodeForBase64");
result.Add(ESAPI);